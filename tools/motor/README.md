...
本项目实现了一个电机的抽象类，与C++中虚类类似，当子类被定义后父类的函数指针会指向子类的定义的函数，只需要调用父类的函数，而无需关心实际子类是如何具体实现的

## 包含文件

项目包含一对`motor.c/h`文件以及相应的`m3508.c`、`m2006.c`、`m6020.c`、`dc_motor.c`、`servo.c`子类文件，使用前将`motor`文件及其需要的子类文件添加至项目中即可

## 依赖项

项目对标准库文件`stdint.h`、`stdlib.h`、`string.h`有依赖，使用了其中的`abs`函数以及`uint32_t`等基础类型，同时也对freertos的`cmsis_os.h`和本项目的`config.h`有依赖，分别使用了读取配置表里的参数初始化电机以及freertos的软件定时器
