...
- **设置定时器配置**：`/tim/setting`
    ...
    | `enable`    | `bool`    | × | 可选 | 开启或关闭定时器 |
    | `channel-x` | `uint8_t` | × | 可选 | 定时器的通道几(与比较值信息绑定) |
    | `compare-value` | `uint32_t` | × | 可选 | PWM的占空比(与定时器通道信息绑定) |
    | `auto-reload`   | `uint32_t` | × | 可选 | 设置定时器重装载值 |
