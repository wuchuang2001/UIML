...
void IMU_TaskCallback(void const * argument)
{
  /* USER CODE BEGIN IMU */
  ...
  for(;;)
  {
    IMU_Bmi088_Update(&IMU);
    IMU.imuAngleRad[1] = Quaternion_getYaw(&IMU.quat);//返回yaw值
		IMU.imuAngleRad[2] = Quaternion_getPitch(&IMU.quat);//返回roll值基于装配变化
		IMU.imuAngleRad[0] = Quaternion_getRoll(&IMU.quat);//返回pitch值

		IMU.imuAngleRad[1] = (Calculate_AngleReset(IMU.imuAngleRad[1],0.f,2*PI));
		IMU.imuAngleRad[0] = (Calculate_AngleReset(IMU.imuAngleRad[0],-PI,PI));
		IMU.imuAngleRad[2] = (Calculate_AngleReset(IMU.imuAngleRad[2],-PI,PI));

		for(uint8_t i=0;i<3;i++)
		{
			IMU.imuAngle360_last[i]=IMU.imuAngle360[i];
			IMU.imuSpeed360[i] = IMU.imuSpeedRad[i] * (180.f / PI);
			IMU.imuAngle360[i] = IMU.imuAngleRad[i] * (180.f / PI);
		}
		osDelay(1);
  }
  /* USER CODE END IMU */
